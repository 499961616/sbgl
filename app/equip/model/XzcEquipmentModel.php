...
class XzcEquipmentModel extends  Model
{
    protected $type = [
        'more' => 'array',
    ];
    ...
    {
        $id = $this->max('id');
        return $lastId = $this->where('id', $id)->value("equip_id");
    }

    //获取仪器编号区间最后一条的id
    public function getInterId($id, $num)
    {
        //例：仪器编号为 1900492y 则要分割两个部分 数字1900492 和 字母y  数字加1  字母不变
        $newLast = substr($id, 7, 1);
        $newFirst = substr($id, 0, 7);
        return $newId = $newFirst + $num - 1 . $newLast;
    }
}
